// Плоская схема документа: `doc` — последовательность блоков, блок не содержит блоков.
//
// Так задумано, а не так вышло. Дерево ProseMirror по умолчанию разрешает абзац внутри пункта
// списка, пункт внутри пункта, абзац внутри цитаты — и каждая такая вложенность это ещё одна
// форма одного и того же содержимого, которую обязаны понимать сериализатор, команды тулбара,
// перетаскивание и курсор. Здесь вложенности нет ни одной:
//
//   - список — цельный блок, его пункты лежат плоским рядом, а «вложенность» это число
//     `depth` на пункте: Tab меняет цифру, а не переставляет узлы;
//   - пункт хранит строчное содержимое напрямую, абзаца внутри него не существует;
//   - цитата — такой же строчный блок, а не контейнер: многострочная цитата это несколько
//     соседних блоков с одним флагом.
//
// Отсюда следует главное свойство: один блок ↔ одна строка markdown, и сериализация
// перестаёт быть догадкой о том, как свернуть дерево.
  ...
  addAttributes() {
    return {
      ...
      checked: {
        ...
        renderHTML: (attributes) => (attributes.checked === null ? {} : { 'data-checked': String(attributes.checked) }),
      },
    }
  },
...
// Язык лежит в атрибуте узла, но на `<code>`, куда его кладёт Tiptap, CSS смотреть не умеет:
// шапку рисует `<pre>`. Декорация переносит язык на внешний элемент — тогда шапка появляется
// сама и исчезает у блока без языка.
function codeLanguages(): Plugin {
  return new Plugin({
    props: {
      decorations(state) {
        const decorations: Decoration[] = []
...
// Собранный набор: подключается вместо blockquote/bulletList/orderedList/listItem из StarterKit.
export const FlatBlocks = Extension.create({
  name: 'flatBlocks',

  addExtensions() {
    return [Quote, List, ListItem]
  },
