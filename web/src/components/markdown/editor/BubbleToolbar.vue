      ...
      <VCard elevation="8" rounded="lg" class="bubble__card">
        ...
        <VMenu location="bottom start">
          ...
          <VList density="compact" nav>
            ...
            <VListItem :active="is('quote')" @click="setBlock('quote')">
        ...
        <button class="bubble__btn" :class="{ 'is-on': is('bold') }" title="Полужирный" @click="chain()?.toggleBold().run()">
          <IconBold :size="16" />
        </button>
        <button class="bubble__btn" :class="{ 'is-on': is('italic') }" title="Курсив" @click="chain()?.toggleItalic().run()">
          <IconItalic :size="16" />
        </button>
        <button class="bubble__btn" :class="{ 'is-on': is('strike') }" title="Зачёркнутый" @click="chain()?.toggleStrike().run()">
          <IconStrikethrough :size="16" />
        </button>
        <button class="bubble__btn" :class="{ 'is-on': is('code') }" title="Код" @click="chain()?.toggleCode().run()">
          <IconCode :size="16" />
        </button>

        <VMenu v-model="linkMenu" location="bottom" :close-on-content-click="false">
          <template #activator="{ props: menu }">
        ...
        <button class="bubble__btn" :class="{ 'is-on': is('quote') }" title="Цитата" @click="setBlock('quote')">
          <IconQuote :size="16" />
        </button>
        <button class="bubble__btn" title="Сбросить форматирование" @click="chain()?.unsetAllMarks().run()">
        ...
        <VMenu v-model="refMenu" location="bottom end" :close-on-content-click="false">
          <template #activator="{ props: menu }">
